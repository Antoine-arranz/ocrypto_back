...
class ChartService {
  ...
  > {
    ...
    const assets = await getQuantityName(walletId);
    ...
    await Promise.all(
      assets.map(async (currency) => {
        const startAt = await findFirstEventByAsset(
          walletId,
        ...
        walletValue.push({
          price: assetTimestamp.map((date, index) => {
            ...
            return price && price.lastState * result.data.prices[index][1];
          }),
        });
      })
    );
    ...
    const usdAmount = timestamps.map((date) => {
      const findDate = new Date(date);
      const usdAmount = ensure(
        events.find((asset: Event) => {
