...
class ChartService {
  ...
  function getArrayOfTimestamp(startDate, stopDate) {
   ...
   const currencies = quantity.map((currency) => {
    return {currency : currency.Currency.symbol.toUpperCase(), name : currency.Currency_Id};
  });
    ...
    const timestamps = startDate.date && getArrayOfTimestamp(startDate.date,new Date())
